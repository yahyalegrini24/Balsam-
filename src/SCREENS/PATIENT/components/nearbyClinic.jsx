...
import { View, Text, Image, StyleSheet, FlatList, Dimensions, TouchableOpacity, ActivityIndicator } from 'react-native'; // Import ActivityIndicator
...
export default function NearbyClinics() {
  ...
  useEffect(() => {
    const fetchClinics = async () => {
      ...
      try {
        ...
        const ClinicsRef = collection(db, 'users'); // Assuming 'users' collection contains user data
        const q = query(ClinicsRef, where('role', '==', 'Professional')); // Filter by role 'Clinic'
        ...
        querySnapshot.forEach((doc) => {
          ...
          if (data.location) {
            ...
            if (distance <= 30) { // Only consider Clinics within 30 km
              ClinicsList.push({
                id: doc.id,
                ...data,
                distance, // Add the calculated distance to the Clinic data
              });
            }
          }
        });

        // Limit to 5 Clinics and sort by distance
        const limitedClinics = ClinicsList
          .sort((a, b) => a.distance - b.distance)
          .slice(0, 5); 

        setClinics(limitedClinics); // Set filtered Clinics
      } catch (error) {
  ...
  const renderClinicItem = ({ item }) => (
    <View style={styles.card}>
      ...
      <TouchableOpacity style={styles.viewProfileButton} onPress={() => navigation.navigate('ClinicProfile', { Clinic: item })}>
    ...
    <View style={styles.container}>
      <View style={styles.headerContainer}>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('NearbyClinic')}
        >
