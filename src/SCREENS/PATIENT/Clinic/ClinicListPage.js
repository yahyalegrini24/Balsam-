...
const ClinicList = ({ route, navigation }) => {
  const { service, onSelectClinic } = route.params; // service.title will be used for filtering
  ...
  useEffect(() => {
    const fetchClinics = async () => {
      try {
        const ClinicsRef = collection(db, 'users'); // Assuming 'users' is the collection name
        const q = query(
          ClinicsRef,
          where('role', '==', 'Clinic'),
          where('services', 'array-contains', service.title) // Filter Clinics based on the service.title
        );
  ...
  const renderClinicItem = ({ item }) => (
    <TouchableOpacity
      style={[
        styles.card,
        selectedClinic?.id === item.id && styles.selectedCard,
      ]}
    ...
    >
      ...
      <View style={styles.ClinicInfo}>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('ClinicProfile', { Clinic: item })}
        >
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.headerTitle}>Available Clinic</Text>
      </LinearGradient>
      {loading ? (
      ...
      >
        <Text style={styles.chooseButtonText}>
          {selectedClinic ? 'Confirm Selection' : 'Choose a Clinic'}
        </Text>
      </TouchableOpacity>
    </View>
  );
