...
const getTotalAppointments = async (userId) => {
  ...
  const q = query(appointmentsRef, where('selectedProfessional', '==', userId));
...
const getPendingRequests = async (userId) => {
  const appointmentsRef = collection(db, 'bookings');
  const q = query(appointmentsRef, where('selectedProfessional', '==', userId), where('status', '==', 'Pending'));
  const querySnapshot = await getDocs(q);
  return querySnapshot.size;
};
...
const getAcceptedAppointments = async (userId) => {
  const appointmentsRef = collection(db, 'bookings');
  const q = query(appointmentsRef, where('selectedProfessional', '==', userId), where('status', '==', 'Active'));
  const querySnapshot = await getDocs(q);
  return querySnapshot.size;
};
const getCompletedAppointments = async (userId) => {
  const appointmentsRef = collection(db, 'bookings');
  const q = query(appointmentsRef, where('selectedProfessional', '==', userId), where('status', '==', 'Completed'));
  const querySnapshot = await getDocs(q);
  return querySnapshot.size;
};
...
const getPayingAppointments = async (userId) => {
  const appointmentsRef = collection(db, 'bookings');
  const q = query(appointmentsRef, where('selectedProfessional', '==', userId), where('isPaid', '==', true));
  const querySnapshot = await getDocs(q);
  return querySnapshot.size;
};
